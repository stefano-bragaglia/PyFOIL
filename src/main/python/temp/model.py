...
class Alpha:
    ...
    def notify(self, fact: Literal, substitution: Substitution, parent: Root):
        substitution = self.pattern.unify(fact)
        if substitution is not None:
            payload = ([fact], substitution)
            if payload not in self.memory:
                self.memory.append(payload)
                for child in self.children:
                    child.notify([fact], substitution, self)
...
class Beta:
    ...
    def notify(self, fact: List[Literal], substitution: Substitution, parent: Node):
        if parent is self.parent_1:
            for ground_2, subs_2 in self.parent_2.memory:
                self._notify(fact, substitution, ground_2, subs_2)
        elif parent is self.parent_2:
            for ground_1, subs_1 in self.parent_1.memory:
                self._notify(ground_1, subs_1, fact, substitution)
    ...
    def _notify(self, fact_1: List[Literal], substitution_1: Substitution,
                fact_2: List['Literal'], substitution_2: Substitution):
