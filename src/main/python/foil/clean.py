...
def learn_hypotheses(
    ...
    hypotheses = []
    while positives:
        result = learn_clause(background, hypotheses, target, masks, positives, negatives)
        if result is None:
            break
...
def learn_clause(
    ...
    key = (tuple(background), tuple(hypotheses), target, tuple(masks), tuple(positives), tuple(negatives))
    if cache and key in _tabling_learn_clause:
        return _tabling_learn_clause[key]

    body = []
    while negatives and not is_over(target, body):
        result = find_literal(background, hypotheses, target, body, masks, positives, negatives)
        if result is None:
            break
...
def find_literal(
    ...
    key = (tuple(background), tuple(hypotheses), target, tuple(body), tuple(masks), tuple(positives), tuple(negatives))
    ...
    for mask in masks:
        for items in itemize(variables, mask.arity):
            ...
            if literal != target and literal not in body and Clause(target, [*body, literal]) not in hypotheses:
